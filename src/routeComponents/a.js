...
import staking_step_2 from "../imgs/staking_step_2.png";
...
export const A = () => {
  return <div>
    ...
    <Section
      ...
      a={<div>
        <p>1. 钱包上的钱才是你真正控制的钱，交易所永远存在跑路或者破产的风险，对于hodl型用户，钱包是个更好的选择</p>
        <p>2. 质押的ADA可以获得奖励，目前年化利率在4%左右。</p>
        <p>3. 为了整个网络的安全性。如果一个PoS网络总质押的代币很少，那么攻击网络则会更容易。目前ADA的总质押率保持在70%，100%左右，较为安全。</p>
      </div>}
    />
    <Section
      q="Q: 质押ADA的安全吗？有锁仓时间吗？"
      a={<div>
        <p>这两个问题的简短的答案是：安全且没有锁仓时间。</p>
        <p>ADA的质押不会将代币转移到质押池。当我们选择质押到某一个池子的时候，会生成一个质押证书，这个质押证书代表我们把这个钱包的钱的代理给质押池。由于没有真正转移ADA代币，所以质押的本金还在我们手中。</p>
        <p>ADA的质押没有锁仓时间。质押的机制为每个纪元结束后进行快照，然后之后会根据快照进行奖励分发。</p>
      </div>}
    />
    <Section
      q="Q: 如何进行质押？"
      a={<div>
        ...
        <p>2. 点击第一个结果，然后点击"Add to Chrome"。</p>
        <img style={{width: "100%"}} src={staking_step_1}/>
        <p>{`3. 安装完成后创建一个Eternl wallet钱包，点击 "Add Wallet" -> "Create wallet" ，输入钱包名字和密码。`}</p>
        <img style={{width: "100%"}} src={staking_step_3}/>
        <p>{`4. 输入钱包的个数，一个助记词可以生成多个钱包，一般只要一个钱包即可。`}</p>
        <img style={{width: "100%"}} src={staking_step_4}/>
        <p>5. 下一步是关于助记词的提示。<span style={{color: 'orange'}}>助记词非常重要，不要泄露,丢失助记词就是丢失了你自己的资产。</span></p>
        <img style={{width: "100%"}} src={staking_step_5}/>
        <p>6. 点击Continue，记录下助记词。<span style={{color: 'orange'}}>请将助记词记录在纸上并做好备份，存在电脑或者云盘有被黑的风险。</span></p>
        <img style={{width: "100%"}} src={staking_step_6}/>
        <p>7. 下一步验证刚才记下的助记词，这步虽然麻烦，但是可以确认你抄下的助记词是正确。</p>
        ...
        {/* <p>{`在币安购买到ADA代币之后。点击，资金 => 现货 => 提现 => 选择ADA => 通过区块链转账，将上面的地址填入地址输入框，转账网络选择"Cardano网络"，然后输入提现金额，验证后既可以提现。币安提现ADA到"Cardano网络"需要1 ADA手续费，一般需要等待5分钟左右能收到ADA代币（币安会将多币交易打包成一笔省手续费，所以需要花点时间）。`}</p>
        <p>在接收到币安发送的代币之后，此时你的ADA已经在钱包里，但是并没有进行质押，我们还需要执行质押的操作才是质押成功。点击staking，在Filter Pools这里搜索质押池子，如何选择质押池请看todo，在这里如果你愿意支持我们的China Love Cardano Pool，可以搜索ChinaLoveCardanoPool。搜索到你要质押的池子后，点击Delegate进行质押。</p> */}
      </div>}
    />
